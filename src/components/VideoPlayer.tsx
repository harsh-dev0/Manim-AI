...
const VideoPlayer: React.FC<VideoPlayerProps> = ({ videoUrl }) => {
  ...
  if (!videoUrl) {
    return (
      <div className="flex flex-col items-center justify-center w-full h-[200px] sm:h-[300px] md:h-[400px] rounded-lg bg-slate-900/30 border border-slate-800 p-4 sm:p-6">
        ...
        <p className="text-xs sm:text-sm text-slate-400 mt-1 sm:mt-2 text-center">
          Generate an animation to see the visualization
        </p>
      </div>
    )
  }
  ...
  return (
    <div className="w-full mx-auto rounded-lg overflow-hidden border border-slate-800 bg-slate-900/30 p-0 relative">
      ...
      {/* Only show error overlay when there's an error */}
      {error && (
        <div className="absolute inset-0 flex items-center justify-center bg-slate-900/80 z-10 pointer-events-none">
          <div className="flex flex-col items-center text-center p-4 sm:p-6">
            ...
            <p className="text-xs sm:text-sm text-slate-300/70 mt-1 sm:mt-2">
      ...
      <style jsx global>{`
        /* Simplified video controls styling to avoid conflicts */
        video::-webkit-media-controls {
          padding: 4px;
        }

        /* Remove the custom panel background that might be causing the blue overlay */
